...
void	fin_dinner(t_philo **philo)
{
	...
	{								//status sonlanan child processin return değerini(nasıl sonlandığını) tutar
		waitpid(-1, &status, 0);	//herhangi bir child process sonlanana kadar bekle
		if (status != 0)		//child process sağlıklı şekilde bitmediyse
		{
			while (i < tmp->num_philo)
			{
				kill(tmp->philo_id[i], SIGKILL); //burada kill ile child process sonlandırılır
				i++;
			}
			break ;
		}
